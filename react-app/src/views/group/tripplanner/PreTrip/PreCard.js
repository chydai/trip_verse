...
import { TextField, Dialog, Select, MenuItem, Button, ButtonGroup, Card, AppBar, Box, CssBaseline, Grid, Toolbar, useMediaQuery, Typography, Icon, CardContent } from '@mui/material';
...
const PreCard = () => {
    ...
    const handleAddDate = () => {
        if (!value) {
            ...
            const isDuplicate = allPlans.find(plan => plan.date === value.toISOString().slice(0, 10))
            if (isDuplicate) {
                setIsError(1)
                ...
                dispatch(addNewPlan({ date: formattedDate, channelId: channelId }))
                    .then((newPlan) => {
            ...
            <Card
                ...
                <CardContent  >
                    <Grid container spacing={gridSpacing}>
                        ...
                        {isError === 1 &&
                            (<Grid item xs={12}>
                                <Alert onClose={() => { setIsError(0) }} severity="error">This is an error alert — Duplicated Itinerary Date!</Alert>
                            </Grid>)
                        }
                        {isError === 2 &&
                            (<Grid item xs={12}>
                                <Alert onClose={() => { setIsError(0) }} severity="error">This is an error alert — Itinerary Date Not Selected!</Alert>
                            </Grid>)
                        }
                            ...
                            allPlaces.map((plan) => {
                                return (
                                    <Grid key={plan._id} item xs={12}>
                                        <EditableCard key={plan._id} title={plan.name} content={plan.note} placeId={plan._id} imgUrl={plan.imgUrl} />
                                    </Grid>)
                            }
                            )
                        }
                        ...
                        <Grid item xs={12}>
                            <AddNewCard title="Add New Place.." content="Add Notes.." planId={curPlan} />
                        </Grid>
                    </Grid>
                </CardContent>
            </Card>
