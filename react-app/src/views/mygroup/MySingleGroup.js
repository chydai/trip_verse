...
import { Dialog, DialogTitle, DialogContent, 
    ...
    FormControl, InputLabel, Select, FormHelperText } from '@mui/material';

import { deleteMyGroup, transferGroup, fetchUserGroups, exitGroup, selectUserGroups } from 'store/groupSlice';
    ...
    return (
        <Grid item key={group._id} xs={12} sm={6} md={6} lg={4}>
            <Card sx={{
                ...
                <CardActionArea onClick={(event) => handleClickItem(event, `/group-page/${group._id}`)}>
                    <CardMedia
                        component="img"
                        height="130"
                ...
                <CardActions disableSpacing sx={{ mt: 0, pt: 0 }}>
                    <IconButton aria-label="add to favorites" disabled>
                        ...
                        <Typography  sx={{color: 'gray'}} variant="button">{group.likes.length}</Typography>
                    </IconButton>
                    
                    <IconButton aria-label="group size" disabled>
                        <GroupIcon sx={{color: 'gray'}}/>
                        <Typography  sx={{color: 'gray'}} variant="button">{group.members.length}</Typography>
                    </IconButton>
