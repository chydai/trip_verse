...
const MyGroup = () => {
    ...
    useEffect(() => {
        ...
        .then((myOwned) => {
          return dispatch(fetchUserGroups(curUser._id))
            .then((response) => {
            ...
            <Grid container spacing={gridSpacing}>
                <Grid item xs={12}>
                    <SubCard title="My Joined Group">
                        <Grid container spacing={gridSpacing}>
                            {onlyJoined.map((group) => (<MySingleGroup key={group._id} group={group} join={true}/>))}
                        </Grid>
                    </SubCard>
                </Grid>
                <Grid item xs={12}>
                    ...
                    <SubCard title="My Owned Group">
                        <Grid container spacing={gridSpacing}>
                            {myOwned.map((group) => (<MySingleGroup key={group._id} group={group} join={false} />))}
                        </Grid>
                    </SubCard>
