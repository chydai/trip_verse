...
export const transferGroup = createAsyncThunk(
  ...
  async (transInfo) => {
    const response = await api.transferGroup(
      transInfo.groupId,
      transInfo.userId
    );
    return response.data;
...
export const removeGroupMember = createAsyncThunk(
  ...
  async (transInfo) => {
    const response = await api.removeMemberFromGroup(
      transInfo.groupId,
      transInfo.userId
    );
    return response.data;
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(updateGroup.fulfilled, (state, action) => {
        let index = state.groupslist.findIndex(
      ...
      .addCase(deleteMyGroup.fulfilled, (state, action) => {
        ...
        let index2 = state.userGroup.findIndex(
          (group) => group._id === action.payload._id
        );
